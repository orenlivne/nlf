...
\end{equation}
$N$ the arclength residual. Block-eliminating $\delta\alpha$, \begin{equation}\label{eq:deflate}
\delta\alpha=\frac{\dot\phi^{\top}w-N}{\dot\alpha+\dot\phi^{\top}u},\qquad
\delta\phi=\delta\alpha\,u-w .
\end{equation}
where $u=J^{+}d$ and $w=J^{+}r$ are two applications of the inner Laplacian solver. Both $u,w\in\mathrm{range}(J^{+})=\chi^{\perp}$: the near-null cut component of the solution (the $\chi$ direction) is pinned by the scalar arclength constraint (second row in \eqref{eq:deflate}). The border keeps the bordered
system nonsingular through the fold ($\kappa=\mathcal{O}(10^3)$ while $\kappa(J)\to\infty$), allowing the inner Laplacian solver to stay fast (LAMG+ cycle ACF $\approx0.01$; similarly for approximate Cholesky's PCG).
...
\State $\alpha_{\mathrm{hi}} \gets \sum_{e\sim s}c_e \ge F^*$ (source-cut bound); set $\alpha\gets0.1\,\alpha_{\mathrm{hi}}$, halve until Alg.~\ref{alg:fixed} converges (ensures $\alpha<F^*$); fix $g>1$ ($g{=}1.7$)
\While{\textbf{true}} \Comment{geometric climb brackets $F^*$}
  \State $(\phi',\ \textit{converged}) \gets$ Algorithm~\ref{alg:fixed} at load $g\,\alpha$, warm-started from $\phi$
  \If{\textbf{not} \textit{converged}}
    \State \textbf{break} \Comment{$g\alpha \ge F^*$: no equilibrium exists, so the fixed-$\alpha$ solve cannot converge}
  \EndIf
  \State $\alpha \gets g\,\alpha$, $\ \phi \gets \phi'$ \Comment{keep the largest feasible load}
\EndWhile
