...
The whole nonlinear solve costs $2$--$4$ linear Laplacian solves. On single-commodity congestion
over real road-network topologies (BPR cost), NLF converges for all $2{,}003$ SuiteSparse graphs
up to $5.6\times10^7$ edges; against a state-of-the-art interior-point method (IPM), NLF was
$2.6\times$ faster on average and $>45\times$ on the poorly-separable graphs where the IPM's
direct KKT core is superlinear; against matrix-free L-BFGS it was $4.2\times$ faster on average
and the only solver to finish on the $90$ hardest instances. A multicommodity extension routes $K$
...
Only the saturating/fold class needs continuation. For the no-fold congestion class the target
$\alpha$ is given, so Algorithm~\ref{alg:fixed} solves \eqref{eq:master} directly at that load,
warm-started from $\phi=0$---no ramp (\S\ref{sec:corpus}); the rest of
this section concerns the fold.
...
vector, $\|\dot\phi\|^2+\dot\alpha^2=1$. Differentiating \eqref{eq:master} along $s$ gives it:
...
$\kappa(J)\to\infty$), so the inner Laplacian solver stays fast (LAMG+ cycle ACF $\approx0.01$;
likewise approximate Cholesky's PCG).
...
Frank--Wolfe~\cite{frankwolfe} converges only sublinearly: the iteration count to a $10^{-4}$
duality gap grows with size (Table~\ref{tab:fw}, $115$ at $n{=}144$) and the $\mathcal{O}(1/k)$
rate cannot reach the high accuracy NLF attains. The field's high-accuracy standard, the
\emph{bush-based} family (Algorithm~B~\cite{bargera2002}, TAPAS~\cite{bargera2010}), solves the
...
benchmark quantitatively against \textbf{Ipopt}~\cite{ipopt}, a mature interior-point solver
on the \emph{identical} program. This is conservative: Ipopt attains the same $10^{-10}$, whereas
...
These NLF times still carry the three-stage load ramp; solving congestion directly at the target load
(Algorithm~\ref{alg:fixed}) is ${\approx}2\times$ faster corpus-wide (\S\ref{sec:corpus})---$2.7\times$
fewer Newton cycles, halved in wall-clock by shared setup---narrowing Ipopt's planar-road lead toward
parity and shifting the crossover to smaller, better-separated graphs.
...
V-cycle at forcing $0.05$; range $6$--$33$; Fig.~\ref{fig:corpus}B). This sweep ramps the load in three
stages; re-run as a single direct solve at the target (no continuation, Algorithm~\ref{alg:fixed}) it
still converges on \emph{all $2{,}003$}, in a median $6$ cycles ($2.7\times$ fewer), the congestion
default. We prove no convergence
...
We compare the three paradigms one could plug into the same outer loop: near-linear (NLF), sparse-direct
...
method inherits). We ran all three on the same BPR program on $1{,}669$ corpus graphs, each competitor
capped at $5\times$ NLF's wall-clock (Table~\ref{tab:robustness}).
\textbf{NLF converged on all $1{,}669$}; first-order failed on $481$ (ill-conditioning,
...
$2.6\times$/$4.2\times$ faster than Ipopt/L-BFGS and fastest-or-only on $85\%$; here L-BFGS already
solved directly at the target load while these NLF times carry the ramp, so dropping it
(\S\ref{sec:bprscale}) only widens NLF's margin over first-order. The ablation
(\S\ref{sec:ablation}) picks the engine within the near-linear class.
...
At the fold, continuation is mandatory ($J$ singular at $F^*$), but each arclength step is only a nested
initial guess~\cite[\S8.3.2]{brandt-guide}---it need only stay on the solution path, so resolving every
corrector to $10^{-9}$ over-solves the \emph{value}. The max-flow $F^*$ is verifiable against
push--relabel (\S\ref{sec:maxflow}) only to ${\approx}10^{-4}$; matching the per-step corrector
tolerance to that reaches the same min cut in $2$--$7\times$ fewer corrector solves and hierarchy
rebuilds, while a single corrector step per arclength step overshoots $F^*$.
