...
\label{alg:cont}
\begin{algorithmic}[1]
\Require demand $d$; a target load $\alpha$ (\textbf{Case~A}) or the request to maximize $\alpha$ (\textbf{Case~B})
\State $\phi \gets 0$, $\ H \gets \emptyset$ \Comment{trivial solution at $\alpha=0$}
\Statex
\State \textbf{Case~A --- congestion, $\alpha$ given.} For $\ell=\tfrac14,\tfrac12,1$: $\phi \gets$ Algorithm~\ref{alg:fixed} at load $\ell\alpha$, warm-started from $\phi$, sharing $H$. \textbf{Return} $\phi$.
\Statex
\State \textbf{Case~B --- max-flow, $\alpha$ unknown (find $F^*$).}
...
\emph{bush-based} family (Algorithm~B~\cite{bargera2002}, TAPAS~\cite{bargera2010}), was
unavailable for a same-instance head-to-head (AequilibraE failed to build; a minimal
Algorithm~B reference did not reach high accuracy), so we position it qualitatively and
benchmark quantitatively against \textbf{Ipopt}~\cite{ipopt}, a mature interior-point solver
on the identical program. This is conservative: Ipopt attains the same $10^{-10}$, whereas
...
L-BFGS ties NLF, so the speed-up over Ipopt there is a fill-in effect any matrix-free method
inherits). We ran all three on the same BPR program on $1{,}669$ corpus graphs, each competitor
...
$2.6\times$/$4.2\times$ faster than Ipopt/L-BFGS and fastest-or-only on $85\%$. The ablation
...
\subsection{How accurately to solve each continuation step}\label{sec:contin-ablation}
Continuation is a nested initial-guess generator~\cite[\S8.3.2]{brandt-guide}: intermediate stages need
only stay on the solution path. On the no-fold congestion class a single warm-from-zero solve---no
continuation at all---reaches $10^{-9}$ in $5$--$9$ inner cycles across a $20\times$ demand range, so the
load ramp is pure overhead. At the fold, continuation is mandatory ($J$ singular at $F^*$), yet resolving
each arclength corrector to $10^{-9}$ over-solves the \emph{value}: matching the per-step corrector
tolerance to the $10^{-4}$ at which $F^*$ is verifiable against push--relabel (\S\ref{sec:maxflow})
reaches the same min cut in $2$--$7\times$ fewer corrector solves and hierarchy rebuilds, while a single
corrector step per arclength step overshoots $F^*$. Work should track the accuracy that matters.
