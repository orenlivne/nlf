...
weighted graph Laplacian system (\ref{eq:jac}), solved by a graph Laplacian solver of choice (e.g., LAMG+ or approximate Cholesky).
\item \emph{Freeze the hierarchy (lazy-setup reuse).} Across a chain of nearby linearizations the AMG
hierarchy is reused rather than rebuilt (note: this is different from
Brandt's frozen-$\tau$~\cite[\S15]{brandt-guide}): touch the setup rarely, and let an observed-convergence monitor
