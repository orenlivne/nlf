...
varied; we parametrize it by its own \emph{arclength} $s$ and write a dot for the derivative along the
curve, $\dot{(\cdot)}\equiv d(\cdot)/ds$, so the tangent $(\dot\phi,\dot\alpha)$ is a \emph{unit}
