...
\label{alg:cont}
\begin{algorithmic}[1]
\State Set $\alpha\gets 0$, $\phi \gets 0$, $H \gets \emptyset$.
\State \textbf{No fold ($\alpha$ given):}
...
\State \textbf{Fold ($\alpha$ unknown, find $F^*$):} Climb $\alpha$ geometrically (each load by Algorithm~\ref{alg:fixed}) until infeasible.
