...
For the no-fold congestion class, $\alpha$ is a given target and the continuation steps $\alpha$
directly: three fixed steps $\ell=\tfrac14,\tfrac12,1$ of the target load suffice to globalize
convergence. Each step warm-starts from the previous solution and shares the same hierarchy state
...
\begin{algorithm}[t]
\caption{NLF continuation (both classes start at $\alpha=0$, $\phi=0$, $H$ empty).
  \textbf{Left:} no-fold / congestion ($\alpha$ given): $\alpha$-continuation with fixed steps.
  \textbf{Right:} fold / max-flow ($\alpha$ unknown): pseudo-arclength continuation to $F^*$.}
\label{alg:cont}
\begin{algorithmic}[1]
\State \textbf{No fold ($\alpha$ given):} $\phi\gets0$, $H\gets\emptyset$.
  For $\ell=\tfrac14,\tfrac12,1$: run Algorithm~\ref{alg:fixed} at load $\ell\alpha$,
  warm-started from previous $\phi$, $H$ shared. \textbf{return} $\phi$
\Statex
\State \textbf{Fold ($\alpha$ unknown, find $F^*$):} $\phi\gets0$, $H\gets\emptyset$.
  Climb $\alpha$ geometrically (each load by Algorithm~\ref{alg:fixed}) until
