...
\emph{independent of $k$}. A solver that returns only the equilibrium must instead recover this gradient
by finite differences---$k$ equilibrium re-solves---so the framework is asymptotically $O(k)$ cheaper
and reaches design problems (thousands of tolled links, $10^6$ demand entries) the re-solve loop cannot.
A directed single-commodity prototype confirms this on optimal tolling---adjoint gradient versus finite
difference, with orders-of-magnitude scaling from Sioux Falls to Anaheim
(\url{https://github.com/orenlivne/dnlf}). The undirected study here is thus a stepping stone: the same
