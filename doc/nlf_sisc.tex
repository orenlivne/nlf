...
\paragraph{When the setup is refreshed} Two conditions trigger a refresh \emph{before} computing the correction $\delta$: (i) no setup yet; (ii) the per-step residual factor exceeded $0.25$---the frozen $J$ has drifted too far from the current linearization. A third case arises \emph{after} the line search: (iii) if all $50$ halvings fail and $H$ was stale (built in a prior iteration), we rebuild $H$ from the current $J(\phi)$, recompute $\delta$, and retry the line search; if the retry still fails, or if $H$ was already fresh, we break. Measured across the corpus of \S\ref{sec:corpus} the number of refreshes is small (often just one). A lazy setup refresh is $1.5$--$4.7\times$ faster than refreshing on every Newton step.
