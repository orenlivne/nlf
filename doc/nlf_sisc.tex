...
sense~\cite{nash-mgopt,gratton-rmtr,brandt-ron-mgopt}: it does not build coarse \emph{optimization} models or recurse
the objective. It reduces the equilibrium to a \emph{single} nonlinear equation \eqref{eq:master} and
employs multigrid (if LAMG+ is used) only as the linear inner solve inside a frozen-linearization chord-Newton
continuation; the nonlinearity is carried by the edge law and the outer continuation, not by a
coarse-grid objective. However, Sec.~\ref{sec:dcopt} outlines how NLF might possibly be extended to a multilevel optimization paradigm when a cost functional is added.
...
node $i$ is the head of edge $e$, $-1$ if the tail). Write $\phi\in\R^n$ for the \emph{node
...
strictly monotone \emph{edge law} $\rho_e:\R\to\R$:
...
\cite{daitch-spielman} likewise wrap a Laplacian solve in a barrier path-following loop. NLF instead places the nonlinearity in the \emph{constitutive law}: $f=\rho(B^{\top}\phi)$ is a
saturating function of the gradient, so the single energy of \eqref{eq:master} encodes the
capacity \emph{exactly} and its feasibility limit is the \emph{exact} min cut. The outer loop is then a short continuation through one scalar fold, not a reweighting or
...
nonlinear Laplacian, each solved by the same linear Laplacian inner solve.

\section{The NLF algorithm}\label{sec:algorithm}
NLF is a continuation loop over the load $\alpha$, starting at $\alpha = 0$, where each step's solution serves as the initial guess to the next $\alpha$-step. Each fixed-$\alpha$ subproblem is solved by a handful of inexact inner solves (each to a tolerance proportional to the current nonlinear residual). The resulting correction is accepted through a line search (damped, inexact chord-Newton iteration). The inner solver's setup---its factor or hierarchy---is built once, frozen across $\alpha$-steps and re-formed only when an observed-convergence monitor signals it has drifted too far. The numbers of steps and inner solves are both bounded, implying empirical-$O(m)$ complexity. Below we describe the linear engine
...
The outer loop treats the inner solver as a black box and asks three things of it: (i)~it solves a
graph-Laplacian system $L\phi=b$ in near-linear $\Om$ work; (ii)~its accuracy is tunable, so it can run
\emph{inexactly} to the forcing tolerance $\eta\|r\|$ of the chord-Newton step (\S\ref{sec:fas}) rather
than to machine precision; and (iii)~its \emph{setup} is separable from its \emph{solve}, so the setup
is built once at a frozen linearization and reused---applied to the nonlinear residual---across many
continuation and Newton steps. The formulation and the frozen-Newton/continuation outer method are the
contribution; the inner solve is an interchangeable module, and any solver meeting (i)--(iii) is a
drop-in engine.

Two public near-linear solvers meet all three with different internals. \emph{Approximate
Cholesky}~\cite{approxchol} \textbf{sets up} a randomized incomplete sparse-Cholesky factor of
$L$---a preconditioner, $\Om$ to build---and \textbf{solves} by preconditioned conjugate gradients to
the requested tolerance. \emph{LAMG+}~\cite{lamgplus} \textbf{sets up} an aggregation-multigrid
hierarchy (relaxation-based affinity aggregation, low-degree Schur elimination, a guarded coarse
operator) and \textbf{solves} by a fractional V-cycle with min-residual recombination; its setup and
per-cycle solve are each $\Om$. In both, setup is the freezable object of requirement (iii) (the
factor, the hierarchy) and the solve is the tunable object of (ii). We take \textbf{approximate
Cholesky as the default}: the ablation of \S\ref{sec:ablation} finds it the faster default across the
corpus at equal robustness. LAMG+ is interchangeable in \emph{robustness}---its wall-clock constant
differs by graph class---and its cut-respecting hierarchy gives a structurally clean near-singular
solve at the fold (next paragraph).
...
no fold-aware tuning. This is a feature of the LAMG+
...
pass of the loop costs one $\Om$ residual evaluation and one $\Om$ inner solve (run inexactly, to
the forcing fraction $\eta\|r\|$, $\eta=0.05$~\cite{inexact-newton}); the expensive setup is paid
only on refresh. The inner solve applies an approximate \emph{inverse} of the frozen linearization,
$\delta\approx J_H^{-1}r$---the (frozen-)Newton direction. With a fresh
setup and an exact solve the step would be exactly Newton's; frozen and inexact, it is a
...
\label{alg:fixed}
\begin{algorithmic}[1]
\Require load $\alpha$; warm start $\phi$; inner-solver state $H$ (factor or hierarchy, shared across calls); relative
...
\paragraph{When the setup is refreshed} Three triggers, all observed-convergence based, no
tuning: (i) no setup yet; (ii) the per-step residual factor exceeded $0.25$---the frozen $J$ has
drifted too far from the current linearization; (iii) the line search stalled on a stale setup
(refresh, retry once; only a fresh-setup stall terminates). Measured across the corpus of
